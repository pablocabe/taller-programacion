...
function buscarListaF (L: lista; patenteX: integer): integer;
begin
	buscarListaF := -1;
	while (L <> nil) and (buscarListaF = -1) do
	begin
		if (patenteX = L^.dato.patente) then
			buscarListaF := L^.dato.modelo;
		L := L^.sig;
	end;
end;

function incisoF (a2: arbol2; patenteX: integer): integer; // tiene que haber una forma más sencilla de hacer este módulo
var
	modelo: integer;
begin
	if (a2 = nil) then
		incisoF := -1 // si el arbol esta vacio devuelve -1
	else begin
		modelo := buscarListaF (a2^.dato.autosLista, patenteX);
		if (modelo <> -1) then
			incisoF := modelo
		else begin
			incisoF := incisoF (a2^.HI, patenteX);
			if (incisoF = -1) then
				incisoF := incisoF (a2^.HD, patenteX);
		end;
	end;
end;
...
begin
	...
	writeln ('Ingrese otra patente de la cual se informara su modelo de auto');
	readln (patenteX);
	writeln ('El modelo del auto de la patente ', patenteX, ' es: ', incisoF (a2, patenteX));
end.
