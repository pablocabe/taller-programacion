...
function calcularCantidadG (L: lista): integer;
begin
	calcularCantidadG := 0;
	while (L <> nil) do
		begin
			calcularCantidadG := calcularCantidadG + 1;
			L := L^.sig;
		end;
end;

procedure agregarAdelanteG (ISBNAux: integer; var L: listaIncisoG; cantidadAux: integer); // lo ordeno por ISBN de mayor a menor, no especifica el enunciado el orden
var
	nuevo: listaIncisoG;
begin
	new (nuevo);
	nuevo^.dato.ISBN := ISBNAux;
	nuevo^.dato.cantidadTotal := cantidadAux;
	nuevo^.sig := L;
	L := nuevo;
end;

procedure retornarIncisoG (a2: arbol2; var listaG: listaIncisoG);
var
	cantidadAux: integer;
begin
	if (a2 <> nil) then
		begin
			retornarIncisoG (a2^.HI, listaG);
			cantidadAux := 0;
			cantidadAux := calcularCantidadG (a2^.dato.listaPrestamos);
			agregarAdelanteG (a2^.dato.ISBN, listaG, cantidadAux);
			retornarIncisoG (a2^.HD, listaG);
		end;
end;

procedure recorrerIncisoG (listaG: listaIncisoG); // lo imprimo en orden inverso a la listaG
begin
	if (listaG <> nil) then
		begin
			recorrerIncisoG (listaG^.sig);
			writeln ('El ISBN ', listaG^.dato.ISBN, ' se presto un total de: ', listaG^.dato.cantidadTotal, ' veces');
		end;
end;
...
begin
	...
	listaG := nil;
	retornarIncisoG (a2, listaG);
	recorrerIncisoG (listaG);
end.
