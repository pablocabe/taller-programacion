...
function prestamosSocio (a1: arbol1; socio: integer): integer;
begin
	if (a1 = nil) then
		prestamosSocio := 0 // el árbol está vacío
	else begin
		prestamosSocio := 0;
		if (socio = a1^.dato.socio) then
			prestamosSocio := prestamosSocio + 1;
		prestamosSocio := prestamosSocio + prestamosSocio (a1^.HI, socio);
		prestamosSocio := prestamosSocio + prestamosSocio (a1^.HD, socio);
	end;
end;
...
begin
	...
	readln (socio);
	writeln ('El socio numero ', socio, ' realizo ', prestamosSocio (a1, socio), ' prestamos'); // inciso D
end.
