program Cap5Ej4d; 

type

	registroLectura = record
		codigo: integer;
		DNI: integer;
		anio: integer;
		tipoReclamo: integer;
	end;
	
	registroLista = record
		codigo: integer;
		anio: integer;
		tipoReclamo: integer;
	end;
	
	lista = ^nodo2;
	nodo2 = record
		dato: registroLista;
		sig: lista;
	end;

	registroArbol = record
		DNI: integer;
		cantidadReclamos: integer;
		listaReclamos: lista;
	end;
	
	arbol = ^nodo1;
	nodo1 = record
		dato: registroArbol;
		HI: arbol;
		HD: arbol;
	end;
	
	listaD = ^nodo3;
	nodo3 = record
		codigo: integer; // es el único elemento que me pide el enunciado
		sig: listaD;
	end;

procedure inicializarArbol (var a: arbol);
begin
	a := nil;
end;

procedure leerReclamo (var reclamo: registroLectura);
begin
	with reclamo do begin
		codigo := random (1000);
		if (codigo <> 0) then
		begin
			DNI := random (100) + 1;
			anio := random (2024) + 1;
			tipoReclamo := random (10) + 1;
		end;
	end;
end;

procedure agregarAdelante (var L: lista; codigo: integer; anio: integer; tipoReclamo: integer);
var
	nuevo: lista;
begin
	new (nuevo);
	nuevo^.dato.codigo := codigo;
	nuevo^.dato.anio := anio;
	nuevo^.dato.tipoReclamo := tipoReclamo;
	nuevo^.sig := L;
	L := nuevo;
end;

procedure crearNodo (var a: arbol; reclamo: registroLectura);
begin
	new (a);
	a^.dato.DNI := reclamo.DNI;
	a^.dato.listaReclamos := nil;
	a^.dato.cantidadReclamos := 1;
	agregarAdelante (a^.dato.listaReclamos, reclamo.codigo, reclamo.anio, reclamo.tipoReclamo);
	a^.HI := nil;
	a^.HD := nil;
end;

procedure agregarOrdenado (var a: arbol; reclamo: registroLectura);
begin
	if (a = nil) then
	begin
		crearNodo (a, reclamo);
	end
	else if (reclamo.DNI = a^.dato.DNI) then
	begin
		a^.dato.cantidadReclamos := a^.dato.cantidadReclamos + 1;
		agregarAdelante (a^.dato.listaReclamos, reclamo.codigo, reclamo.anio, reclamo.tipoReclamo);
	end
	else if (reclamo.DNI > a^.dato.DNI) then
		agregarOrdenado (a^.HD, reclamo)
	else
		agregarOrdenado (a^.HI, reclamo);
end;

procedure cargarArbol (var a: arbol);
var
	reclamo: registroLectura;
	x: integer;
begin
	x := 0;
	leerReclamo (reclamo);
	while (reclamo.codigo <> 0) do
	begin
		agregarOrdenado (a, reclamo);
		x := x + 1;
		leerReclamo (reclamo);
	end;
	writeln ('Se efectuaron ', x, ' reclamos'); // esto lo utilizo para controlar la carga de datos
end;

function recorrerLista (L: lista): integer;
var
	cantidadAux: integer;
begin
	cantidadAux := 0;
	while (L <> nil) do
	begin
		cantidadAux := cantidadAux + 1;
		L := L^.sig;
	end;
	recorrerLista := cantidadAux;
end;

function calcularIncisoB (a: arbol; DNIX: integer): integer;
begin
	if (a = nil) then // si el árbol está vacío o no existe el DNI, devuelve el valor 0
		calcularIncisoB := 0
	else if (DNIX = a^.dato.DNI) then
		calcularIncisoB := recorrerLista (a^.dato.listaReclamos)
	else if (DNIX > a^.dato.DNI) then
		calcularIncisoB := calcularIncisoB (a^.HD, DNIX)
	else
		calcularIncisoB := calcularIncisoB (a^.HI, DNIX);
end;


function calcularIncisoC (a: arbol; DNIA, DNIB: integer): integer;
begin
	if (a = nil) then
		calcularIncisoC := 0
	else if (a^.dato.DNI < DNIA) then
		calcularIncisoC := calcularIncisoC (a^.HD, DNIA, DNIB)
	else if (a^.dato.DNI > DNIB) then
		calcularIncisoC := calcularIncisoC (a^.HI, DNIA, DNIB)
	else
		calcularIncisoC := recorrerLista (a^.dato.listaReclamos) + calcularIncisoC (a^.HI, DNIA, DNIB) + calcularIncisoC (a^.HD, DNIA, DNIB);
end;

procedure agregarAdelanteIncisoD (var LD: listaD; codigo: integer);
var
	nuevo: listaD;
begin
	new (nuevo);
	nuevo^.codigo := codigo;
	nuevo^.sig := LD;
	LD := nuevo;
end;

procedure recorrerListaIncisoD (L: lista; anioX: integer;  var LD: listaD);
begin
	while (L <> nil) do
	begin
		if (anioX = L^.dato.anio) then
			agregarAdelanteIncisoD (LD, L^.dato.codigo);
		L := L^.sig;
	end;
end;

procedure cargarListaIncisoD (a: arbol; anioX: integer; var LD: listaD);
begin
	if (a <> nil) then
	begin
		cargarListaIncisoD (a^.HI, anioX, LD);
		recorrerListaIncisoD (a^.dato.listaReclamos, anioX, LD);
		cargarListaIncisoD (a^.HD, anioX, LD);
	end;
end;

var
	a: arbol;
	DNIX, DNIA, DNIB, anioX: integer;
	LD: listaD;
begin
	randomize;
	inicializarArbol (a);
	cargarArbol (a);
	writeln ('Ingrese un DNI: ');
	readln (DNIX);
	writeln ('La cantidad de reclamos efectuados por el DNI ', DNIX, ' es: ', calcularIncisoB (a, DNIX));
	writeln ('Ingrese el DNI minimo del rango a buscar: ');
	readln (DNIA);
	writeln ('Ingrese el DNI maximo del rango a buscar: ');
	readln (DNIB);
	writeln ('La cantidad de reclamos efectuados por los DNI comprendidos entre ', DNIA, ' y ', DNIB, ' es: ', calcularIncisoC (a, DNIA, DNIB));
	writeln ('Ingrese un anio: ');
	readln (anioX);
	LD := nil;
	cargarListaIncisoD (a, anioX, LD);
end.

{4. Una oficina requiere el procesamiento de los reclamos de las personas. De cada reclamo
se ingresa código, DNI de la persona, año y tipo de reclamo. El ingreso finaliza con el
código de igual a 0. Se pide:
a) Un módulo que retorne estructura adecuada para la búsqueda por DNI. Para cada DNI
se deben tener almacenados cada reclamo y la cantidad total de reclamos que realizó.
b) Un módulo que reciba la estructura generada en a) y un DNI y retorne la cantidad de
reclamos efectuados por ese DNI.
c) Un módulo que reciba la estructura generada en a) y dos DNI y retorne la cantidad de
reclamos efectuados por todos los DNI comprendidos entre los dos DNI recibidos.
d) Un módulo que reciba la estructura generada en a) y un año y retorne los códigos de
los reclamos realizados en el año recibido.}
