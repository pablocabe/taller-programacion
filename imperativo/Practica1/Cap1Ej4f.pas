...
function calcularPromedio (v3: vector3; dL: integer; valorTotal: real): real;
begin
	calcularPromedio := valorTotal / dL;
end;
...
begin
    ...
    while (pos < dL) do // (v3[pos].rubro = 3) no lo toma
		begin
			pos := pos + 1;
			valorTotal := valorTotal + v3[pos].precio;
			writeln ('El precio del producto ubicado en la ', pos, ' del vector 3 es: ', v3[pos].precio: 2: 2);
		end;
	writeln ('El precio promedio de los productos del vector 3 es: ', calcularPromedio (v3, dL, valorTotal): 2: 2);
end;
